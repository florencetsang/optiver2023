...
class EnrichDFDataPreprocessor(DataPreprocessor):
    ...
    def apply(self, df):
        ...
        # df_['wap_240'] = df.groupby(['stock_id', 'date_id'])['wap'].transform(lambda s: s.rolling(24, min_periods=12).mean())
...
class MovingAvgPreProcessor(DataPreprocessor):
    ...
    # def _get_col_name(self, window, min_periods):
    #     return f"mov_avg_{window}_{min_periods}"  # e.g. mov_avg_3_1
    
    # def _get_orig_idx_col_name(self, window, min_periods):
    #     col_name = self._get_col_name(window, min_periods)
    #     return f"orig_df_idx_{col_name}"  # e.g. orig_df_idx_mov_avg_3_1
    ...
    # def _calc_mov_avg(self, grouped_df, consolidated_mov_avg_df, window, min_periods):
    #     orig_idx_col_name = self._get_orig_idx_col_name(window, min_periods)
    ...
    #     consolidated_mov_avg_df[orig_idx_col_name] = mov_avg_df.index
    #     consolidated_mov_avg_df[mov_avg_col_name] = mov_avg_df[self.feature_name]

    #     return consolidated_mov_avg_df
    
    # def _enrich_orig_df(self, orig_df, consolidated_mov_avg_df, window, min_periods):
    #     mov_avg_col_name = self._get_mov_avg_col_name(window, min_periods)
    #     orig_df[mov_avg_col_name] = consolidated_mov_avg_df[mov_avg_col_name]
    #     return orig_df
    ...
    def apply(self, df):
        ...
        # consolidated_mov_avg_df = self._calc_mov_avg(grouped_df, consolidated_mov_avg_df, 3, 1)
        # consolidated_mov_avg_df = self._calc_mov_avg(grouped_df, consolidated_mov_avg_df, 6, 3)

        # consolidated_mov_avg_df = consolidated_mov_avg_df.sort_values(by=["orig_df_idx_mov_avg_3_1"])

        # df = self._enrich_orig_df(df, consolidated_mov_avg_df, 3, 1)
        # df = self._enrich_orig_df(df, consolidated_mov_avg_df, 6, 3)
